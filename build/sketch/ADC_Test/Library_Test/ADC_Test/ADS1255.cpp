...
uint32_t ads1255::read_single24(void){
   ...
   * Remember: Positive full scale is 7FFFFFh, negative 800000h.
   */
  ...
  if(_Input_buffer_24.ui8[2] > 0x7F)                //ADS1255 Positive full scale is 0x7FFFFF, LSB=2*Vref/(PGA(2^23-1)), Binary Two Complement format, anything bigger than 7F must be negative number
    _Input_buffer_24.ui8[3] = 0xFF;                 //Fill negative number MSB with FF, remember, data is sent in 32-bit format, rather than ADC's 24-bit.
  else
    _Input_buffer_24.ui8[3] = 0x00;                 //... and MSB = 0x00 when its positive.
