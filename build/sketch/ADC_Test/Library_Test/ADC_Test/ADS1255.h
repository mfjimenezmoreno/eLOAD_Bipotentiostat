...
class ads1255{
  public:
    ...
    uint32_t read_fast24(void);              //For reading data when continous operation
    uint32_t read_single24(void);            //For reading data on-demand
  ...
  private:
    ...
    union Buffer _ADC_buffer;
    union Buffer _Input_buffer;        //16 bit Buffers
    ...
    union Buffer32 _Input_buffer_24;                  //32 bit buffer for readout
};
